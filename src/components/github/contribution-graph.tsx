...
const getMonthLabels = (weeks: Week[], monthNames: string[] = DEFAULT_MONTH_LABELS): MonthLabel[] => {
  ...
  return labels.filter(({ weekIndex }, index, filteredLabels) => {
    ...
    if (index === 0) {
      return filteredLabels[1] && filteredLabels[1].weekIndex - weekIndex >= minWeeks
    }
...
export const ContributionGraphTotalCount = ({ className, children, ...props }: ContributionGraphTotalCountProps) => {
  ...
  return (
    <div className={cn('text-dim-label', className)} {...props}>
      {labels.totalCount
...
export const ContributionGraphLegend = ({ className, children, ...props }: ContributionGraphLegendProps) => {
  ...
  return (
    <div className={cn('ml-auto flex items-center', className)} style={{ gap: blockMargin }} {...props}>
      <span className="mr-1 text-dim-label">{labels.legend?.less || 'Less'}</span>
      ...
      <span className="ml-1 text-dim-label">{labels.legend?.more || 'More'}</span>
    </div>
  )
}
